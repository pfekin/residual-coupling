    ...
    def __init__(self, dim, num_models, mode):
        ...
        for i in range(num_models):
            for j in range(num_models):
                ...
                self.projections[f"{j}_to_{i}"] = nn.Sequential(
                    nn.Linear(dim, dim//2), 
                    nn.Linear(dim//2, dim)
                )
                """
        ...
        # ABLATION: Random Projection Bridge (Freeze the weights)
        if "random" in mode:
...
class DifferanceEngine(nn.Module):
    ...
    def forward(self, ids):
        ...
        for l in range(C["layers"]):
            ...
            # BASELINE: Logit Ensemble bypasses latent bridges entirely
        ...
        # ISOLATION ROUTING:
        # If hybrid or ensemble, mix the logits.
        # Otherwise, output only the Generalist (Model A) which has absorbed the bridged knowledge.
        ...
        for item in med_ds:
            ...
            for choice in choices:
                ...
                if isinstance(outputs, tuple):
                    # outputs[1] contains the list of steered logits for all models
...
def run_comprehensive_sweep():
    ...
    with torch.no_grad():
        ...
        print(" Evaluating Gen (A) on TruthfulQA...")
        base_tqa_a = run_truthfulqa_eval(model_A, tokenizer) if RUN_TRUTHFUL_QA else 0.0

        spec_baselines = []
        for i, spec in enumerate(specs):
            print(f" Evaluating Spec (B{i+1}) on PPL & TruthfulQA...")
            spec_ppl = get_ppl(spec(torch.clamp(test_ids, 0, spec.config.vocab_size-1)).logits, test_ids)
            spec_tqa = run_truthfulqa_eval(spec, tokenizer) if RUN_TRUTHFUL_QA else 0.0
            spec_baselines.append((i+1, spec_ppl, spec_tqa))
    ...
    modes = [
        ...
        "star_bilateral", 
        "multi_bilateral",            
        "hybrid_multi_bilateral",     
        "multi_bilateral_no_gate",    
        "multi_bilateral_random",     
        "moe"
    ]
    ...
    for mode in modes:
        ...
        with torch.no_grad():
            ...
            # 1. Evaluate Generalist / Fused Output (Index 0)
            ppl_A = get_ppl(final_l, test_ids)
            tqa_A = run_truthfulqa_eval(net, tokenizer, target_idx=0) if RUN_TRUTHFUL_QA else 0.0
            results[mode] = {"ppl": ppl_A, "tqa": tqa_A}
            
            # 2. Evaluate Steered Specialist 1 (Index 1)
            if len(logits_list) > 1:
                results[mode]["ppl_B"] = get_ppl(logits_list[1], test_ids)
                results[mode]["tqa_B"] = run_truthfulqa_eval(net, tokenizer, target_idx=1) if RUN_TRUTHFUL_QA else 0.0
                
            # 3. Evaluate Steered Specialist 2 (Index 2)
            if len(logits_list) > 2:
                results[mode]["ppl_C"] = get_ppl(logits_list[2], test_ids)
                results[mode]["tqa_C"] = run_truthfulqa_eval(net, tokenizer, target_idx=2) if RUN_TRUTHFUL_QA else 0.0
        
        del net; torch.cuda.empty_cache(); gc.collect()

    # --- FINAL JOURNAL-READY REPORT ---
    print("\n" + "═"*115)
    print(f" SYNERGY-X: STEERED MULTI-AGENT PERFORMANCE (Test Samples: {TEST_SAMPLES})")
    print("─"*115)
    print(f" {'MODE':<25} | {'GEN (A) PPL/TQA':<22} | {'SPEC (B) PPL/TQA':<22} | {'SPEC (C) PPL/TQA':<22}")
    print("─"*115)
    
    # Print Baselines
    str_base_A = f"{base_ppl_a:<7.2f} / {base_tqa_a:>5.2f}%"
    str_base_B = f"{spec_baselines[0][1]:<7.2f} / {spec_baselines[0][2]:>5.2f}%"
    str_base_C = f"{spec_baselines[1][1]:<7.2f} / {spec_baselines[1][2]:>5.2f}%"
    print(f" {'FROZEN BASELINES':<25} | {str_base_A:<22} | {str_base_B:<22} | {str_base_C:<22}")
    print("─"*115)
