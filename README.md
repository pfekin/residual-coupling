...
* **Mitigating Idiosyncratic Hallucinations:** Because the bridge projections are strictly linear, they can only map existing geometric structures between the representation spaces. As the bridges are optimized against ground-truth target data, they have no incentive to map ungrounded features such as individual models' hallucinations, effectively drowning them out as noise while reinforcing the factual signals.
