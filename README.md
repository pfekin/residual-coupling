...
* **Inherent Hallucination Suppression:** Because the bridge projections are linear, their capacity to represent arbitrary mappings is sharply constrained compared to non-linear layers, limiting what they learn to the linear structure already present in the representation spaces. As the bridges are optimized strictly against ground-truth target data, they have no incentive to map ungrounded features such as individual models' hallucinations. Uncorrelated errors across independent base models fail to find a valid geometric alignment toward the target objective, effectively drowning them out as noise while reinforcing factual signal.
