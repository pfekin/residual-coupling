...
Residual Coupling (RC) implements a different approach. Frozen transformers function as non-linear memorizers: their weights define the boundaries of what the system can know. At each bridge layer, a small learned projection reads one model's hidden state and adds a corrective update directly into the other model's residual stream, the same stream that each transformer layer already writes into additively. The bridge does not replace or intercept the receiving model's computation, it perturbs the stream that computation reads from. The receiving model then processes that perturbed stream through its own frozen weights, unchanged. Bridge projections are linear operators that navigate the structured differences between those memorized spaces. Because they are linear, their capacity to represent arbitrary mappings is sharply constrained compared to a non-linear layer, so what they learn is limited to the linear structure already present in the relationship between the two frozen models' representation spaces.
...
Transformers memorize. Their weights encode a compressed non-linear map from inputs to outputs, defined by what the model saw during training. Two models trained on different corpora encode different things, but in geometrically compatible ways: the relative positions of concepts in representation space are approximately preserved across independently trained models even when their coordinate systems differ. This compatibility strengthens as models scale. The bridge does not teach one model what the other knows. It learns to navigate the geometric relationship between what each has separately memorized.
Freezing both models before bridge training begins has a precise consequence. A frozen model does not change to accommodate its input. Every input is processed through the same invariant weights, regardless of what the bridge injects into the residual stream. The bridge update is assimilated through existing computation, not through modified parameters. Catastrophic forgetting is not mitigated here. It is absent. The condition that produces it, updating weights in response to new data, never occurs.
The same property has a bearing on hallucination, though the evidence is narrower. Each model trained on different data carries its own idiosyncratic errors: outputs that are confident and wrong because the training corpus had gaps or distributional artifacts. That noise is uncorrelated across independently trained models. During bridge training, the gate scalars learn to amplify projected components that produce consistent updates on both sides and to suppress components that appear on only one side. The TruthfulQA results are consistent with this. Factual accuracy improves alongside perplexity across all RC topologies, and all topologies outperform MoE on both metrics. The result comes from one domain and one evaluation set and should be read as a hypothesis about mechanism rather than a documented regularity.

## Where this points

Progress in language modeling has largely meant making individual models larger. RC adds a different axis. A generalist is trained and frozen. A specialist is trained on a distinct domain or modality and frozen. Bridges are trained between them. A second specialist is added. The cycle does not stop at a fixed ensemble size. Each specialist is trained on the manifold established by all preceding frozen models. Each can be deactivated by removing its bridges in reverse order of addition, leaving everything prior intact and recoverable without retraining. Because the coupled models execute in parallel, inference time is bounded by the slowest model in the ensemble regardless of how many specialists are coupled.
Whether a collection of small frozen specialists coupled through bridges can match or exceed the capability of a single large model trained on everything is an open question. The three-model results show a gap between selection-based and coupling-based approaches that does not close as the ensemble grows: MoE at 0.5% perplexity reduction, multi-bilateral at 80.7%. That result comes from one domain. Whether the gap widens with more models is what the next experiments will determine.
Neuroscience has long proposed that the cortex is organized as functionally specialized columns that operate independently yet produce unified cognition through lateral connections. Each frozen model in RC is the column: a memorizer that encodes one regime of the world. The bridges are the lateral connections. The training regime implied is iterative: column by column, bridge by bridge, each addition leaving what came before unchanged.
Because the coupled models communicate through hidden state tensors at bridge layers rather than through their weights, a specialist on a local or edge device and a generalist on a remote server would exchange activations, not weights. Neither party needs to expose its model. Bridge parameters encode only the geometric relationship between two frozen models on a training distribution and travel independently of either. What new approaches to deployment and model lifecycle management become possible at larger scale is what further research will determine.
