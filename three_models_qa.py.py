...
if __name__ == "__main__":
    run_comprehensive_sweep()import torch
import torch.nn as nn
import torch.nn.functional as F
from transformers import AutoModelForCausalLM, AutoTokenizer
from datasets import load_dataset
    ...
    with torch.no_grad():
        for item in med_ds:
            ...
            for choice in choices:
                ...
                outputs = net(ids)
                logits = outputs[1][target_idx] if isinstance(outputs, tuple) else outputs
...
def run_comprehensive_sweep():
    ...
    for mode in modes:
        ...
        # Differential learning rates for bridges vs routers
        opt = torch.optim.AdamW([
            {"params": [p for n, p in net.named_parameters() if p.requires_grad and not ("router" in n or "mix" in n)], "lr": 1e-4},
            {"params": [p for n, p in net.named_parameters() if p.requires_grad and ("router" in n or "mix" in n)], "lr": 5e-3}
        ])
